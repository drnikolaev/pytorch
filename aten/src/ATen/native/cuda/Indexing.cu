...
template<typename T, typename IndexType> __device__ __forceinline__ IndexType
...
return offset +
linearIndex* strides[0];
}


template<typename scalar_t>
__global__ void
backward_indexing_kernel(int64_t* extendedIdx, int64_t* origOrder, scalar_t* gradValues,
  ...
  if (beforeIndex.defined() || afterIndex.defined()) {
    ...
    for (int side = 0; side < 2; ++side) {
      ...
      AT_DISPATCH_FLOATING_TYPES(self.scalar_type(), "index_put_cuda_kernel_", [&] {
        ...
        cuda::detail::TensorInfo <scalar_t, int64_t> self_info = cuda::detail::getTensorInfo<scalar_t, int64_t>(
            self);
        ...
        THCudaCheck(cudaLaunchCooperativeKernel(
            (const void*) &backward_indexing_kernel<scalar_t>, gridSize, blockSize, args,
            blockSize * GROUP_SIZE * 2, stream));
